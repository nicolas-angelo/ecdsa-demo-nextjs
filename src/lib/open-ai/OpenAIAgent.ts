...
const log = (desc: string, data: any) => console.log(desc, data);
...
export class OpenAIAgent {
	...
	async runAgent(
		...
		do {
			log('Iteration: ', i);
			response = await this.createChatCompletion(messages, functionsObjects);
			if (response) {
						...
						for (const functionsObject of functionsObjects) {
							...
							if (functionsObject[functionName]) {
								functionResponse = await functionsObject[functionName](
									...Object.values(
										JSON.parse(functionArgs.arguments || '') || {}
									)
								);
								break;
							}
